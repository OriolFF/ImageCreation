...
// API_BASE is set by config.js (generated by serve.py) or defaults to 8000
// config.js defines: API_PORT and API_BASE
const apiBase = (typeof API_BASE !== "undefined") ? API_BASE : "http://127.0.0.1:8050";

const API_URL = `${apiBase}/v1/images/generations`;
const MODELS_URL = `${apiBase}/v1/models`;
const SELECT_MODEL_URL = `${apiBase}/v1/models/select`;
const CANCEL_URL = `${apiBase}/v1/images/cancel`;
const RELEASE_MEMORY_URL = `${apiBase}/v1/memory/release`;
...
async function loadModels() {
  try {
    ...
    console.log("[UI] loadModels: API_BASE is", apiBase);
    ...
    let res;
    try {
      res = await fetch(MODELS_URL, { signal: controller.signal });
    } catch (fetchErr) {
      console.error("[UI] loadModels: fetch threw error:", fetchErr.name, fetchErr.message);
      throw fetchErr;
    }
    clearTimeout(t);
    ...
    const entries = Object.entries(available); // [key, modelConfig]
    for (const [key, modelConfig] of entries) {
      const opt = document.createElement("option");
      opt.value = key;
      // modelConfig is an object like {id: "...", type: "..."}
      const modelId = typeof modelConfig === 'string' ? modelConfig : modelConfig.id;
      opt.textContent = `${key} (${modelId})`;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Set API links dynamically based on API_BASE
  const apiDocsLink = $("#apiDocsLink");
  const healthLink = $("#healthLink");
  if (apiDocsLink) apiDocsLink.href = `${apiBase}/docs`;
  if (healthLink) healthLink.href = `${apiBase}/health`;
...
// Wire up core buttons with null checks
